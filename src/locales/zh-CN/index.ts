export default {
    introduce: {
        title: {
            ...
            h2: {
                datasetAnimation: "数据集动画展示",
                datasetAnnotation: "数据集标注方法展示",
            },
        },

        prompt: "提示词",
        exampleAnimation: "示例动画",
        ...
        examples: [
            ...
            "角色双手握着楔形武器，高举在头顶，右腿在前左腿在后扎马步，然后向下挥砍。\t",
        ],

        content: {
            abstract: `随着人工智能领域的迅速发展，对于高质量、风格化的动作数据集的需求日益增长。目前，学术界普遍使用的只有一个两年前制作的包含14.6k动作的开源动作数据集HumanML3D 。由于动作数量稀少、风格单一化生活化，因此极大限制了研究的深度与广度。为了解决这一问题，我们独立制作并开源了一个全新的、精心标注的、有14.8k动作的风格化战斗动作数据集CombatMotion Dataset(简称"CM数据集")。\n\n传统上，描述一个战斗动作是非常困难且模糊的工作，这在模型训练和自动化生成中会导致语义理解方面的不一致性 。为了克服这一挑战，我们通过规范标准，划分出八大类战斗动作描述维度，并通过逐类建立丰富词库的方式，提高了标注效率和一致性。\n\n通过对近期各类科研成果的综合比较和新数据集的尝试性训练，我们得到结论，MotionGPT(简称"MGPT")框架 在语义理解和一致性方面表现最佳。这是一种统一的动作-语言框架，其核心思路是，将原始的运动数据通过向量量化的自编码器(VQ-VAE)压缩成离散的动作词元(Token)，然后利用动作感知语言模型(motion-aware language model)去学习这些动作词元与自然语言之间的关系。\n\n基于MotionGPT框架和CM数据集，我们成功训练出了一个能够生成高质量战斗动作的文本-动作模型——AnimationGPT(简称"AGPT")，并撰写了基于Maya的动作重定向脚本，将学术界常用的SMPL骨骼 转换为工业界广泛使用的BVH格式。\n\n综上，我们实现了一整套可复制的工作流规范，包括数据提取、数据标注、动作重定向、训练模型、推理和接入商业引擎的全过程。这一突破不仅为运动数据集的丰富化和科研工作的拓宽和深入提供了坚实的基础，也为未来的动作生成技术的发展和商业应用开辟了新的道路。\n\n我们希望这项工作能激励更多的研究者、开发者和企业共同探索这一未充分挖掘的领域，共同推动人工智能和人类动作生成实用化的发展。`,
            method: `为了在动作生成任务中引入大规模语言数据和模型，我们借助了一个统一的动作-语言框架，名为MotionGPT。具体来说，MotionGPT采用人类运动的离散向量量化，并将3D动作转换为动作词元的方法，这类似于词语词元的生成过程。在这种“动作词汇”基础上，以统一的方式对动作和文本进行语言建模，将人类运动视为一种新的外语，借助模型的推理将人类的自然语言“翻译”成最贴切的人类动作序列。`,
            datasets: `我们首先从高质量作品资源中提取了主要为战斗风格的动作数据，这些数据详细记录了各种动作的信息。\n\n为统一动作数据的标准，我们选择了SMPL（Skinned Multi-Person Linear Model）骨骼作为基础，这是一个线性可叠加的通用人体蒙皮骨骼模型，并将所有动作数据重定向到SMPL骨骼，确保了数据的一致性和可复用性。\n\n在动作数据的标注过程中，我们详细划分了包括动作类型、武器类型、攻击类型、方位类型、力量感、速度感、模糊形容词和姿势描述句等八个字段。我们依据策划的常用表述为前七类字段建立了丰富的词库，并按照映射表进行人工标注，同时运用自然语言处理技术优化了标注内容的表达方式。\n\n这些努力使我们得以创建包含一个14.8k个动作及同等数量标注的战斗风格动作数据集——CombatMotionRaw Dataset（CMR Dataset）。通过训练反馈后，我们对数据集大小和标注内容进行了多次调整，最终形成了含有8.7k个动作和26.1k标注文本的精制数据集CombatMotionProcessed Dataset（CMP Dataset）。`,
            datasetAnnotation:
